#include <stdio.h>
#include <conio.h>

int main()
{
    float num = 0;
    float sum = 0;
    float decimalPlace = 1;
    int flagForDecimal = 0;
    char arr[6][8] = {0};
    printf("Enter 6 real numbers\n> ");
    for (int i = 0; i < 6; i++)
    {
        int j = 0; // Intializes to zero so that for next number iterations for the coloumn starts from 0
        while ((arr[i][j] = getche()) != '\r')
        {
            j++;
        }
        arr[i][j] = '\0'; // replaces \n with \0
        if (i == 5)
        {
            continue; // Dont prints a ">" for 7th number
        }
        printf("\n> "); // newline for next number
    }
    printf("\n");
    for (int k = 0; k < 6; k++)
    {
        num = 0;            // Intializes to zero to store next number from starting
        decimalPlace = 0.1; // Intializes to 0.1 for first string then for 2ns string
        flagForDecimal = 0;
        for (int i = 0; arr[k][i] != '\0'; i++) // converts number into an integer till "." (decimal).
        {
            if (arr[k][i] == '.')
            {
                flagForDecimal = 1; // flag becomes one id decimal exist
                continue; // skips the iteration for '.'
            }
            if (flagForDecimal == 0)
            {
                arr[k][i] -= '0';           // Converts the given string into integer value by subtracting its ASCII value
                num = num * 10 + arr[k][i]; // Stores the consecutive element of string OR digit of a number in a variable to make an integer
            }
            if (flagForDecimal == 1) // executes only when decimals remaining after integer
            {
                arr[k][i] -= '0';
                num = num + (float)(arr[k][i] * decimalPlace);
                decimalPlace /= 10; // converts decimal place into 0.01,0.001,and so on.
            }

        }
        sum += num;
    }
    printf("\nThe sum of the given 6 real numbers( given as strings ) is %.2f\n", sum);
    printf("The average of the given 6 real numbers( given as strings ) is %.2f\n", sum/6.0);

    return 0;
}